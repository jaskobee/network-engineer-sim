...
function serializeDevice(dev) {
  return {
    ...
    dns_servers:      [...(dev.dns_servers      ?? [])],
    dhcp_dns_servers: [...(dev.dhcp_dns_servers ?? [])],
    domain_lookup:    dev.domain_lookup,
    powered:          dev.powered          ?? false,
    interfaces:    dev.interfaces.map(i => ({ ...i })),
    routing_table: (dev.routing_table || []).map(r => ({ ...r })),
    vlan_db:       { ...(dev.vlan_db || {}) },
    dhcp_pools:       dev.dhcp_pools       ? dev.dhcp_pools.map(p       => ({ ...p, dns_servers: [...(p.dns_servers ?? [])] })) : undefined,
